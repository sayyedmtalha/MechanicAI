...
def create_hex_bolt(size, length, fastener_type="iso4014"):
    ...
    try:
        ...
        if '-' not in size and size.startswith('M'):
            # Add standard pitch for metric sizes
        ...
        bolt = HexHeadScrew(size=size_with_pitch, length=length, fastener_type=fastener_type)
        return b3d.Part(bolt.wrapped)  # Wrap TopoDS in build123d Part
    except Exception as e:
        print(f"Hex bolt creation failed: {e}")
        return None
...
def create_bearing(bearing_type="ball", size="M8-16-4"):
    ...
    if not BD_WAREHOUSE_AVAILABLE:
        print("BD Warehouse not available")
        return None
    
    try:
        print(f"Creating bearing: type={bearing_type}, size={size}")
        ...
        wrapped_part = b3d.Part(bearing.wrapped)  # Wrap TopoDS in build123d Part
...
def generate_cad_code(model, request: str) -> str:
    ...
    prompt = f"""You are a CAD code generator. Generate Python code to create 3D parts using build123d.

IMPORTANT: Use ONLY the functions listed below for component creation - do NOT use build123d directly except for basic operations.
...
You also have access to build123d (imported as b3d) and math module for other operations.
